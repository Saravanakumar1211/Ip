...
SOURCE_PREFETCH_N   = 4    # only query top-N nearest sources per station
...
def run_optimization():
    ...
    for si, srow in enumerate(needing_rows):
        ...
        # Pre-filter: top SOURCE_PREFETCH_N sources by haversine distance
        ranked = sorted(
            sources_list,
            key=lambda s: haversine(slat, slon, float(s["lat"]), float(s["lon"]))
        )[:SOURCE_PREFETCH_N]
        for src in ranked:
...
def build_excel(delivery_plans, fleet_status, trucks, unserved, output_path):
    wb    = Workbook()
...
if __name__=="__main__":
    delivery_plans, fleet_status, trucks, unserved = run_optimization()
    ...
    build_excel(delivery_plans, fleet_status, trucks, unserved, out_path)
    ...
    if unserved:
        print(f"   ❌ Unserved     : {len(unserved)} stations (see 'Unserved Stations' sheet)")
    print(f"\n   Output        : {out_path}\n")